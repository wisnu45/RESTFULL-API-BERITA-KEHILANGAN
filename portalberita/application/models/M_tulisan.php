...
class M_tulisan extends CI_Model{
	...
	function get_all_tulisan(){
		...
		$response = $this->_client->request('GET','Tulisan',[
            'query' => [
            	'keyberita' => 'berita',
				'tulisan_tanggal' => 'tulisan_id'
            ]
       ]);
	...
	function get_all_tulisan_kirim(){
		//$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d/%m/%Y') AS tanggal FROM tbl_tulisan  where tulisan_status='0' ORDER BY tulisan_id DESC");
		//return $hsl;
	...
	function simpan_tulisan($judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		///$hsl=$this->db->query("insert into tbl_tulisan(tulisan_judul,tulisan_isi,tulisan_kategori_id,tulisan_kategori_nama,tulisan_pengguna_id,tulisan_author,tulisan_gambar,tulisan_slug,tulisan_status) values ('$judul','$isi','$kategori_id','$kategori_nama','$user_id','$user_nama','$gambar','$slug','1')");
		//return $hsl;

		$response = $this->_client->request('POST', 'Tulisan', [
			'form_params' => [
				'tulisan_judul' => $judul,
				'tulisan_isi' => $isi,
	...
	function update_tulisan($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$gambar,$slug){
		//$hsl=$this->db->query("update tbl_tulisan set tulisan_judul='$judul',tulisan_isi='$isi',tulisan_kategori_id='$kategori_id',tulisan_kategori_nama='$kategori_nama',tulisan_pengguna_id='$user_id',tulisan_author='$user_nama',tulisan_gambar='$gambar',tulisan_slug='$slug',tulisan_status='1' where tulisan_id='$tulisan_id'");
		//return $hsl;

		$response = $this->_client->request('PUT', 'Tulisan', [
			'form_params' => [
				'tulisan_judul' => $judul,
				'tulisan_isi' => $isi,
				'tulisan_kategori_id' => $kategori_id,
	...
	function update_tulisan_tanpa_img($tulisan_id,$judul,$isi,$kategori_id,$kategori_nama,$user_id,$user_nama,$slug){
		//$hsl=$this->db->query("update tbl_tulisan set tulisan_judul='$judul',tulisan_isi='$isi',tulisan_kategori_id='$kategori_id',tulisan_kategori_nama='$kategori_nama',tulisan_pengguna_id='$user_id',tulisan_author='$user_nama',tulisan_slug='$slug',tulisan_status='1' where tulisan_id='$tulisan_id'");
		//return $hsl;
	...
	function hapus_tulisan($kode){
		...
		$response = $this->_client->request('DELETE', 'Tulisan', [
			'form_params' => [
				'keyberita' => 'berita',
				'tulisan_id' => $kode
			]
	...
	//						--Menmapilkan Data Pada Home---

	function get_post_home(){
		$hsl=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d %M %Y') AS tanggal FROM tbl_tulisan where tulisan_status='1' ORDER BY tulisan_id DESC limit 50");
		return $hsl;
	}
    ...
    function get_tulisan_populer(){
		$hasil=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d %M %Y') AS tanggal FROM tbl_tulisan ORDER BY tulisan_views DESC limit 50");
		return $hasil;
	}

	function get_tulisan_terbaru(){
		$hasil=$this->db->query("SELECT tbl_tulisan.*,DATE_FORMAT(tulisan_tanggal,'%d %M %Y') AS tanggal FROM tbl_tulisan ORDER BY tulisan_id DESC limit 50");
		return $hasil;
	}
